...
const BusTicket = () => {
  ...
  const handleApplyCoupon = async () => {
    if (selectedSeats.length === 0) {
      toast.warning("আগে সিট নির্বাচন করুন!");
      return;
    }
    if (!couponCode.trim()) return;
    setCouponApplying(true);
    try {
      const { data } = await api.post('/bookings/apply-coupon', { couponCode, subtotal });
      if (data.valid) {
        setAppliedCoupon(couponCode);
        setDiscountAmount(data.discountAmount);
        toast.success("কুপন সফলভাবে অ্যাপ্লাই হয়েছে!");
      }
    } catch (error) {
      setAppliedCoupon('');
      setDiscountAmount(0);
      toast.error(error.response?.data?.message || "অবৈধ কুপন কোড");
    } finally {
  ...
  const handlePaymentSuccess = (tranId) => {
    ...
    toast.success("বুকিং সফল হয়েছে! পেমেন্ট গেইটওয়ে সম্পন্ন হয়েছে।", {
      position: "top-center",
      autoClose: 3000,
    });
    setSelectedSeats([]);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
          ...
          <div className="lg:col-span-5">
            <motion.div layout className="bg-gradient-to-b from-[#0f172a] to-[#0a1120] border border-emerald-500/20 rounded-[3rem] p-10 text-white shadow-[0_20px_50px_rgba(0,0,0,0.5)] sticky top-10">
              ...
              {/* Coupon Section */}
              <div className="mb-4 bg-[#070b14] p-4 rounded-2xl border border-white/5">
                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2 block">Coupon Code</label>
                <div className="flex gap-2">
                  <input type="text" value={couponCode} onChange={e => setCouponCode(e.target.value)} disabled={appliedCoupon !== ''} className="flex-1 bg-slate-900 border border-white/10 p-3 rounded-xl outline-none text-sm text-center text-white focus:border-emerald-500/50" placeholder="e.g. RUET20" />
                  <button onClick={handleApplyCoupon} disabled={couponApplying || appliedCoupon !== '' || selectedSeats.length === 0} className="bg-slate-800 border border-slate-700 text-white px-4 rounded-xl text-[10px] font-black uppercase hover:bg-emerald-500 hover:border-emerald-400 transition-colors cursor-pointer disabled:cursor-not-allowed">
                    {appliedCoupon ? "Applied" : couponApplying ? "..." : "Apply"}
                  </button>
                </div>
              </div>

              <div className="mb-6 bg-[#070b14] p-5 rounded-2xl border border-emerald-500/20 text-center">
                <p className="text-emerald-400 font-bold text-xs">🔒 Payment will be securely processed by SSLCommerz</p>
              </div>

              <div className="mb-10 text-center bg-[#070b14] py-6 rounded-2xl border border-white/5 relative">
                {discountAmount > 0 && <div className="absolute top-2 right-4 text-emerald-400 text-xs font-bold">-৳{discountAmount} Discount</div>}
